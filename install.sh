...
initVar() {
	...
	# 3.xray-core preview version installation
	coreInstallType=
...
installNginxTools() {
	...
	if [[ "${release}" == "debian" ]]; then
		...
		curl -o /tmp/nginx_signing.key https://nginx.org/keys/nginx_signing.key >/dev/null 2>&
...
initTLSNginxConfig() {
	...
	if [[ -n "${currentHost}" ]]; then
	    echo "Read the last installation record, the domain name of the last installation: ${currentHost}"
		echo
		read -r -p "Please pay attention to the correctness of the detection. Are you using the domain name from the last installation? [y/n]:" historyDomainStatus
...
checkIP() {
	...
	if [[ -z ${localIP} ]] || ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q '\.' && ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q ':'; then
		...
		read -r -p "Should you modify the firewall rules through a script to open the ${portFirewallPortStatus} port? [y/n]:" allPortFirewallStatus
...
acmeInstallSSL() {
	if [[ "${dnsSSLStatus}" == "true" ]]; then
		...
		if [[ -n "${txtValue}" ]]; then
			...
			echoContent yellow " ---> Please refer to this tutorial for adding method, https://github.com/reeceyng/v2ray-agent/blob/master/documents/dns_txt.md"
...
checkCustomPort() {
	if lsof -i "tcp:${customPort}" | grep -q LISTEN; then
		echoContent red "\n ---> ${customPort} port is occupied, please close it manually and install\n"
		lsof -i tcp:80 | grep LISTEN
		exit 0
	fi
}
...
installTLS() {
	...
	if [[ -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" && -f "/etc/v2ray-agent/tls/${tlsDomain}.key" && -n $(cat "/ etc/v2ray-agent/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh /${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
		...
		if [[ ! -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" || ! -f "/etc/v2ray-agent/tls/${tlsDomain}.key" ]] || [ [ -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.key") || -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.crt") ] ]; then
			...
			allowPort 80
			allowPort 443
			echoContent yellow " ---> Try installing the TLS certificate again"
...
nginxBlog() {
	...
	if [[ -d "/usr/share/nginx/html" && -f "/usr/share/nginx/html/check" ]]; then
		...
		if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
			...
			wget -q -P /usr/share/nginx https://raw.githubusercontent.com/reiceyng/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
...
handleXray() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "xray/xray") ]]; then
			...
			echoContent red "Please manually execute [/etc/v2ray-agent/xray/xray -confdir /etc/v2ray-agent/xray/conf] and check the error log"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
		if [[ -z $(pgrep -f "xray/xray") ]]; t hen
...
initHysteriaPort() {
	...
	if [[ -z "${hysteriaPort}" ]]; then
		echoContent yellow "Please enter the Hysteria port [example: 10000], it cannot be repeated with other services"
...
initTrojanGoConfig() {
	...
	echoContent skyBlue "\nProgress$1/${totalProgress}: Initializing Trojan configuration"
...
defaultBase64Code() {
	...
	if [[ "${type}" == "vlesstcp" ]]; then
		...
		if [[ "${coreInstallType}" == "1" ]] && echo "${currentInstallProtocolType}" | grep -q 0; then
			...
			echoContent yellow " ---> Formatted plain text (VLESS+TCP+TLS/xtls-rprx-direct)"
...
EOF
			...
			echoContent yellow " ---> Formatted plain text (VLESS+TCP+TLS/xtls-rprx-splice)"
...
EOF
			...
			echoContent yellow " ---> Formatted plain text (VLESS+TCP+TLS/xtls-rprx-vision)"
			echoContent green "Protocol type: VLESS, address: ${currentHost}, port: ${currentDefaultPort}, user ID: ${id}, security: tls, transmission method: tcp, flow: xtls-rprx-vision, account name: ${email/direct/splice}-xray-vision\n"
...
EOF
		...
		echoContent yellow " ---> Formatted plain text (Trojan+TCP+TLS/xtls-rprx-direct)"
...
EOF
		...
		echoContent yellow " ---> Formatted plain text (Trojan+TCP+TLS/xtls-rprx-splice)"
		echoContent green "Protocol type: VLESS, address: ${currentHost}, port: ${currentDefaultPort}, user ID: ${id}, security: xtls, transmission method: tcp, flow: xtls-rprx-splice, account name: ${email/direct/splice}\n"
		cat <<EOF >>"/etc/v2ray-agent/subscribe_tmp/${subAccount}"
...
EOF
		...
		echoContent green "Protocol type: VLESS, address: ${currentAdd}, disguised domain name/SNI: ${currentHost}, port: ${currentDefaultPort}, user ID: ${id}, security: tls, transmission method: ws, path :/${currentPath}ws, account name:${email}-vless-ws\n"
...
EOF
		...
		echoContent green "Protocol type: VLESS, address: ${currentAdd}, disguised domain name/SNI: ${currentHost}, port: ${currentDefaultPort}, user ID: ${id}, security: tls, transmission method: gRPC, alpn :h2, serviceName:${currentPath}grpc, account name:${email}-vless-grpc\n"
	...
	if [[ -n "${configPath}" ]]; then
		...
		if echo ${currentInstallProtocolType} | grep -q 1; then
			...
			jq .inbounds[0].settings.clients ${configPath}03_VLESS_WS_inbounds.json | jq -c '.[]' | while read -r user; do
				...
				# if [[ ${coreInstallType} == "1" ]]; then
				# echoContent yellow "There will be Xray's 0-RTT path after it. It is not compatible with the client with v2ray as the core. Please delete it manually before using\n"
				# path="${currentPath}ws"
				#fi
		...
		if echo ${currentInstallProtocolType} | grep -q 5; then
			echoContent skyBlue "\n================================ VLESS gRPC TLS CDN ========== =====================\n"
			echoContent red "\n --->gRPC is in the testing phase and may not be compatible with the client you are using. If it cannot be used, please ignore"
			# local serviceName
...
updateNginxBlog() {
	...
	echoContent red "================================================== ==============="
	echoContent yellow "# If you want to customize, please manually copy the template file to /usr/share/nginx/html \n"
	...
	if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /usr/share/nginx "https://raw.githubusercontent.com/reiceyng/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip " >/dev/null
		else
			wget -c -P /usr/share/nginx "https://raw.githubusercontent.com/reiceyng/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		fi
...
unInstall() {
	...
	if [[ -z "${hysteriaConfigPath}" ]]; then
		handleHysteria stop
		rm -rf /etc/systemd/system/hysteria.service
		echoContent green " ---> Deletion of Hysteria is completed after booting up"
	fi
...
updateV2RayAgent() {
	...
	echoContent skyBlue "wget ​​-P /root -N --no-check-certificate https://raw.githubusercontent.com/reiceyng/v2ray-agent/${scriptType}/install.sh && chmod 700 /root/install.sh && /root/install.sh"
	echo
	exit 0
}
...
ipv6Routing() {
	...
	if [[ "${ipv6Status}" == "1" ]]; then
		echoContent red "================================================== ==============="
		echoContent yellow "# Notes\n"
		echoContent yellow "1. Rules only support predefined domain name lists [https://github.com/v2fly/domain-list-community]"
		echoContent yellow "2. Detailed documentation [https://www.v2fly.org/config/routing.html]"
		echoContent yellow "3. If the kernel startup fails, please check the domain name and add the domain name again"
		echoContent yellow "4. Special characters are not allowed, pay attention to the format of commas"
		echoContent yellow "5. Each time you add it, you add it again and the last domain name will not be retained."
...
warpRouting() {
	...
	# echoContent yellow "2. The machine can be used normally without restarting. If you must use the official warp, it is recommended not to restart the machine."
	# echoContent yellow "3. Some machines can still be used normally after restarting"
	# echoContent yellow "4. It cannot be used after restarting. You can also uninstall and reinstall"
	# Install warp
	if [[ -z $(which warp-cli) ]]; then
		echo
	...
	echoContent red "\n================================================ ================="
	echoContent yellow "1.Add domain name"
	echoContent yellow "2. Uninstall WARP distribution"
	echoContent red "================================================== ==============="
	...
	if [[ "${warpStatus}" == "1" ]]; then
		...
		echoContent yellow "1. Rules only support predefined domain name lists [https://github.com/v2fly/domain-list-community]"
		echoContent yellow "2. Detailed documentation [https://www.v2fly.org/config/routing.html]"
		echoContent yellow "3. You can only divert traffic to warp, and cannot specify ipv4 or ipv6"
		echoContent yellow "4. If the kernel startup fails, please check the domain name and add the domain name again"
		echoContent yellow "5. Special characters are not allowed, pay attention to the format of commas"
		echoContent yellow "6. Each time you add it, you add it again and the last domain name will not be retained."
...
dokodemoDoorUnblockStreamingMedia() {
	echoContent skyBlue "\nFunction 1/${totalProgress}: Any door floor machine unlocks streaming media"
	echoContent red "\n================================================ ================="
	echoContent yellow "# Notes"
	echoContent yellow "For details on unlocking any door, please view this article [https://github.com/reiceyng/v2ray-agent/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"
...
#VMess+WS+TLS goes to battle to unlock streaming media [outbound only]
unblockVMessWSTLSStreamingMedia() {
	echoContent skyBlue "\nFunction 1/${totalProgress}: VMess+WS+TLS outbound unlock streaming"
	echoContent red "\n================================================ ================="
	echoContent yellow "# Notes"
...
setDokodemoDoorUnblockStreamingMediaOutbounds() {
	read -r -p "Please enter the IP to unlock streaming vps:" setIP
	echoContent red "================================================== ==============="
	echoContent yellow "# Notes\n"
	echoContent yellow "1. Rules only support predefined domain name lists [https://github.com/v2fly/domain-list-community]"
	echoContent yellow "2. Detailed documentation [https://www.v2fly.org/config/routing.html]"
...
setDokodemoDoorUnblockStreamingMediaInbounds() {
	...
	echoContent yellow "8. Domain name entry examples: netflix, disney, hulu\n"
	read -r -p "Please enter the IP allowed to access the unlocked vps:" setIPs
	if [[ -n "${setIPs}" ]]; then
		read -r -p "Please enter the domain name according to the above example:" domainList
		allowPort 22387
		allowPort 22388
...
EOF
		...
		reloadCore
		echoContent green " ---> Added landing machine inbound unlocking successfully"
		exit 0
	fi
	echoContent red " ---> ip cannot be empty"
}
...
dnsUnlockNetflix() {
	...
	echoContent skyBlue "\nFunction 1/${totalProgress}: DNS Unlock Streaming"
	echoContent red "\n================================================ ================="
	echoContent yellow "1.Add"
	echoContent yellow "2.Uninstall"
	read -r -p "Please select:" selectType
...
# Set dns
setUnlockDNS() {
	read -r -p "Please enter the DNS to unlock streaming media:" setDNS
	if [[ -n ${setDNS} ]]; then
		echoContent red "================================================== ==============="
		echoContent yellow "# Notes\n"
		echoContent yellow "1. Rules only support predefined domain name lists [https://github.com/v2fly/domain-list-community]"
		...
		echoContent yellow "4. Special characters are not allowed, pay attention to the format of commas"
		echoContent yellow "5. Each time you add it, you add it again and the last domain name will not be retained."
		echoContent yellow "6. Input examples: netflix, disney, hulu"
...
hysteriaCoreInstall() {
	if [[ -z "${coreInstallType}" ]]; then
		echoContent red "\n ---> Due to environmental dependencies, if you want to install hysteria, please install Xray/V2ray first"
		menu
		exit 0
	fi
	totalProgress=5
...
subscribe() {
	if [[ -n "${configPath}" ]]; then
		...
		echoContent yellow "# You need to check the subscription again every time you add or delete an account"
...
menu() {
	...
	echoContent yellow "8. Change CDN node"
	echoContent yellow "9.IPv6 offload"
	echoContent yellow "10.WARP offload"
